...
def test_invalid_reveal_leaves_book_and_engine_live():
    ...
    assert asyncio.run(gate.reveal_move("thief", 0, "state", "S", "truth", thief[1], thief[2]))["status"] == "waiting"
...
def test_resolved_payload_uses_revealing_peer_role():
    ...
    gate.submit_commitment("police", 0, police[0], police[2])
    gate.submit_commitment("thief", 0, thief[0], thief[2])
    asyncio.run(gate.reveal_move("thief", 0, "state", "S", "truth", thief[1], thief[2]))
    result = asyncio.run(gate.reveal_move("police", 0, "state", "N", "truth", police[1], police[2]))
