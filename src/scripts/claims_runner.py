...
is ignored rather than rejected. It also runs the pairing check outbound.
...
async def play_series(apps: dict, call, sub_games: int, seed: int, wait,
    ...
    for index, role in enumerate(schedule, start=1):
        ...
        # Per SUB-GAME: the visited set and the STAY run describe this walk,
        # and carrying either across a boundary judges a new board by an old.
        thaw = Thaw(app.policy.role, app.policy.settings.max_consecutive_stay)
        ...
        summary = await play_sub_game(
            ...
            choose=_chooser(app, side, rng, board, thaw),
