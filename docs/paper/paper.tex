...
$u_i(p_{1:T}, e_{1:T}, \sigma_{1:T}) = u_i(p_{1:T}, e_{1:T})$ for both
players. It is therefore \emph{cheap talk}. In a strictly adversarial game the
...
Each half-turn carries $\mathrm{commit} = \mathrm{SHA256}(
\mathrm{canonical}(r) \,\|\, \nu)$ over the sealed record
$r = \{\text{step}, \text{state}, \text{position}, \text{move},
\text{intent}, \text{hint}\}$ with a fresh nonce $\nu$; records and nonces are
disclosed only at the closing audit, where the opponent re-hashes each against
